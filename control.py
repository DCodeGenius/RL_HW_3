...
###### BEGIN YOUR CODE ######
V = np.random.uniform(0, 0.10, NUM_STATES)
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    for a in range(NUM_ACTIONS):
        q_values[a] = np.dot(T[state, a, :], rewards + GAMMA * V)
    ...
    if new_state == NUM_STATES - 1:
        ...
        for s in range(NUM_STATES):
            for a in range(NUM_ACTIONS):
                ...
                if count > 0:
                    T[s, a, :] = transition_counts[s, a, :] / count

        observed_rewards = reward_counts > 0
        rewards[observed_rewards] = reward_sums[observed_rewards] / reward_counts[observed_rewards]
        ###### END YOUR CODE ######
        ...
        while True:
            ...
            for s in range(NUM_STATES):
                q_values = np.zeros(NUM_ACTIONS)
                for a in range(NUM_ACTIONS):
                    q_values[a] = np.dot(T[s, a, :], rewards + GAMMA * old_V)
                V[s] = np.max(q_values)
